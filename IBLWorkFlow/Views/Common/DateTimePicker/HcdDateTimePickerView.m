...
{
    [[IBLAppDelegate sharedDelegate].window.rootViewController.view addSubview:self];
...
-(void)dismissWithCompleteBlock:(DatePickerCompleteAnimationBlock)block{
...
-(void)dismiss:(UITapGestureRecognizer *)tap{
    ...
    if( CGRectContainsPoint(self.frame, [tap locationInView:timeBroadcastView])) {
        ...
        [self dismissWithCompleteBlock:^(BOOL Complete) {
    ...
    [self dismissWithCompleteBlock:^(BOOL Complete) {
        if (btns.tag == kOKBtnTag) {
            ...
            if (self.formatter) {
                ...
                dateTimeStr = [self.formatter stringFromDate:date];
            }
            NSLog(@"date %@",dateTimeStr);
